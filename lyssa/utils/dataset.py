...
def save(data,path=None,prefix=None,suffix=".npy"):
	...
	if type(data) is np.ndarray or type(data) is np.core.memmap:
		...
		#case it is a proxy like
		#the online reader class
		n_files = len(data)
	...
	def __getitem__(self,index):
		...
		if not self.color:
			...
			if len(np.array(img).shape) == 2:
				#case we want RGB but the original image
				#is in grayscale
				img_arr = np.array(img)
...
#For each person,
#we randomly select 20 images for training and the other 6 for testing.
class AR_RandomFace():
	...
	def __call__(self):
		import scipy.io
		file_name = "/media/rewire/New Volume/ML_data/AR_face/randomfaces4ar/randomfaces4ar.mat"
		mat = scipy.io.loadmat(file_name, mdict=None)
		...
		#print mat.keys()
		#print mat['featureMat'].shape
		#print mat['labelMat'].shape
		X = mat['featureMat'][:]
		if self.mmap:
			X = get_mmap(X)
